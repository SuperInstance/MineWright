...
def fix_no_whitespace_before(content: str) -> Tuple[str, int]:
    ...
    patterns = [
        (r'\bif\(', 'if (', 'if'),
        (r'\bwhile\(', 'while (', 'while'),
        (r'\bfor\(', 'for (', 'for'),
        (r'\bswitch\(', 'switch (', 'switch'),
        (r'\bcatch\(', 'catch (', 'catch'),
        (r'\bsynchronized\(', 'synchronized (', 'synchronized'),
    ]

    for pattern, replacement, name in patterns:
        # Only replace if there's no space already
        ...
        if i + 1 < len(lines):
            ...
            if (current.rstrip().endswith(')') or current.rstrip().endswith(';')):
                if next_line == '{':
                    ...
                    # Check if it's a control structure or method
                    ...
                    # Check for method signature (has return type or constructor)
                    is_method = (re.search(r'\b(public|private|protected|static|final|abstract|native|synchronized)\s+',
                                          stripped) or
                                re.search(r'\b(void|int|boolean|String|List|Map|Set|Object|float|double|long|char|byte|short)\s+\w+\s*\(',
                                          stripped) or
                                # Constructor
                                re.search(r'\b[A-Z]\w+\s*\(', stripped) and not stripped.startswith('if '))
...
def get_all_imports(content: str) -> List[Tuple[int, str, str]]:
    """
    Extract all import statements from content.
    Returns list of (line_index, full_import, package) tuples.
    """
    imports = []
    lines = content.split('\n')
...
def main():
    ...
    report_content = f"""# Checkstyle Auto-Fix Report - Week 4
...
- **Java files processed:** {total}
- **Files modified:** {global_stats['files_modified']}
...
|-----------------|--------------|-------------|
| **NoWhitespaceBefore** | {global_stats['nowhitespace']} | Added space before `(` in control structures |
| **LeftCurly** | {global_stats['leftcurly']} | Moved opening brace to same line |
| **ImportOrder** | {global_stats['importorder']} | Sorted imports alphabetically |
| **UnusedImports** | {global_stats['unused']} | Removed unused import statements |
| **Total** | {sum([global_stats['nowhitespace'], global_stats['leftcurly'], global_stats['importorder'], global_stats['unused']])} |  |
