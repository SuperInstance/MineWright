...
public class CrewManager {
    private static final Logger LOGGER = TestLogger.getLogger(CrewManager.class);
    ...
    public ForemanEntity spawnCrewMember(ServerLevel level, Vec3 position, String name) {
        LOGGER.info("Current active crew members: {}", activeCrewMembers.size());

        if (activeCrewMembers.containsKey(name)) {
            LOGGER.warn("Crew member name '{}' already exists", name);
            return null;
        }
        int maxCrewMembers = MineWrightConfig.MAX_ACTIVE_CREW_MEMBERS.get();
        if (activeCrewMembers.size() >= maxCrewMembers) {
            LOGGER.warn("Max crew member limit reached: {}", maxCrewMembers);
            return null;
        }
        ForemanEntity crewMember;
        try {
            LOGGER.info("EntityType: {}", MineWrightMod.FOREMAN_ENTITY.get());
        ...
        try {
            ...
            if (added) {
                ...
                LOGGER.info("Successfully spawned crew member: {} with UUID {} at {}", name, crewMember.getUUID(), position);
                return crewMember;
            } else {
                LOGGER.error("Failed to add crew member entity to world (addFreshEntity returned false)");
                LOGGER.error("=== SPAWN ATTEMPT FAILED ===");
            }
        } catch (Throwable e) {
    ...
    public void clearAllCrewMembers() {
        LOGGER.info("Clearing {} crew member entities", activeCrewMembers.size());
    ...
    public void tick(ServerLevel level) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!crewMember.isAlive() || crewMember.isRemoved()) {
                ...
                LOGGER.info("Cleaned up crew member: {}", entry.getKey());
            }
        }
    }
}
