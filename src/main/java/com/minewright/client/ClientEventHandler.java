...
public class ClientEventHandler {
    ...
    private static void startVoiceListening() {
        ...
        if (!voice.isEnabled()) {
            MineWrightMod.LOGGER.debug("Voice system is disabled - ignoring push-to-talk");
            return;
        }

        try {
            MineWrightMod.LOGGER.info("Push-to-talk activated - starting voice input");
            voice.listenForCommand().thenAccept(transcribedText -> {
                MineWrightMod.LOGGER.info("Voice input transcribed: {}", transcribedText);
                ...
                MineWrightMod.LOGGER.error("Voice input failed", e);
                ForemanOfficeGUI.addSystemMessage("[VOICE] Error: Failed to capture voice input");
                return null;
            });
        } catch (Exception e) {
            MineWrightMod.LOGGER.error("Failed to start voice listening", e);
        }
    }
    ...
    private static void stopVoiceListening() {
        ...
        MineWrightMod.LOGGER.info("Push-to-talk released - stopping voice input");
        voice.stopAll();
    }
}
