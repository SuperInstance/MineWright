...
public class BuildStructureAction extends BaseAction {
    ...
    private CollaborativeBuildManager.CollaborativeBuild collaborativeBuild; // For multi-Foreman collaboration
    ...
    public BuildStructureAction(ForemanEntity foreman, Task task) {
        super(foreman, task);
    }
    ...
    protected void onTick() {
        ...
        if (ticksRunning > MAX_TICKS) {
            foreman.setFlying(false); // Disable flying on timeout
        ...
        if (isCollaborative && collaborativeBuild != null) {
            ...
            for (int i = 0; i < BLOCKS_PER_TICK; i++) {
                ...
                BlockState existingState = foreman.level().getBlockState(pos);
                ...
                // Particles and sound
                if (foreman.level() instanceof ServerLevel serverLevel) {
                    ...
                    var soundType = blockState.getSoundType(foreman.level(), pos, foreman);
                    foreman.level().playSound(null, pos, soundType.getPlaceSound(),
            ...
            if (ticksRunning % 100 == 0 && collaborativeBuild.getBlocksPlaced() > 0) {
                ...
                MineWrightMod.LOGGER.info("{} build progress: {}/{} ({}%) - {} Foremen working",
                    ...
                    collaborativeBuild.getTotalBlocks(),
                    percentComplete,
                    collaborativeBuild.participatingForemen.size());
            }
        } else {
            foreman.setFlying(false); // Disable flying on error
    ...
    protected void onCancel() {
        foreman.setFlying(false); // Disable flying when cancelled
    ...
    private boolean isSolidGround(BlockPos pos) {
        var blockState = foreman.level().getBlockState(pos);
    ...
    private boolean isAreaSuitable(BlockPos startPos, int width, int height, int depth) {
        ...
        for (BlockPos checkPos : checkPoints) {
            ...
            for (int y = 1; y <= Math.min(height, 3); y++) {
                ...
                var blockState = foreman.level().getBlockState(abovePos);
    ...
    /**
     * Gets the current progress percentage (0-100) for this build action.
