 ...
 *
 * This class is exposed to JavaScript code as the `foreman` global object.
 */
public class ForemanAPI {
    private final ForemanEntity foreman;
    private final Queue<Task> actionQueue;

    public ForemanAPI(ForemanEntity foreman) {
    ...
    /**
     * Send a chat message to players in the level.
    ...
    /**
     * Get crew member's current position
     * @return Map with x, y, z coordinates
     */
    public Map<String, Double> getPosition() {
        Vec3 pos = foreman.position();
        ...
        for (int x = -radius; x <= radius; x += 2) {
            for (int y = -radius; y <= radius; y += 2) {
                for (int z = -radius; z <= radius; z += 2) {
                    BlockPos pos = foremanPos.offset(x, y, z);
    ...
    public List<String> getNearbyEntities(int radius) {
        ...
        AABB searchBox = new AABB(
            foremanPos.x - radius, foremanPos.y - radius, foremanPos.z - radius,
            foremanPos.x + radius, foremanPos.y + radius, foremanPos.z + radius
        );
    ...
    /**
     * Check if crew member is idle (no pending actions)
    ...
    /**
     * Get the crew member entity (for internal use)
     */
    ForemanEntity getForemanEntity() {
        return foreman;
    }
}
