...
public class GeminiClient {
    ...
    public GeminiClient() {
        this.apiKey = MineWrightConfig.OPENAI_API_KEY.get(); // We'll use the same config for now
    ...
    public String sendRequest(String systemPrompt, String userPrompt) {
        if (apiKey == null || apiKey.isEmpty()) {
            MineWrightMod.LOGGER.error("Gemini API key not configured!");
            return null;
        }
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                MineWrightMod.LOGGER.error("Gemini API request failed: {}", response.statusCode());
            ...
            if (responseBody == null || responseBody.isEmpty()) {
                MineWrightMod.LOGGER.error("Gemini API returned empty response");
                return null;
            }
            ...
            MineWrightMod.LOGGER.error("Error communicating with Gemini API", e);
            return null;
        }
    }
            ...
            if (json.has("candidates") && json.getAsJsonArray("candidates").size() > 0) {
                ...
                if (firstCandidate.has("finishReason")) {
                    ...
                    if ("MAX_TOKENS".equals(finishReason)) {
                        MineWrightMod.LOGGER.error("Gemini response was cut off due to MAX_TOKENS limit");
                    }
                }
                ...
                if (firstCandidate.has("content")) {
                    ...
                    if (content.has("parts") && content.getAsJsonArray("parts").size() > 0) {
                        ...
                        MineWrightMod.LOGGER.error("Gemini response has no 'parts' in content - response may have been cut off");
                    }
                }
            }

            MineWrightMod.LOGGER.error("Unexpected Gemini response format: {}", responseBody);
            return null;
            
        } catch (Exception e) {
            MineWrightMod.LOGGER.error("Error parsing Gemini response", e);
            return null;
        }
    }
}
