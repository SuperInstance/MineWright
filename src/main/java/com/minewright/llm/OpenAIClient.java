...
public class OpenAIClient {
    ...
    public String sendRequest(String systemPrompt, String userPrompt) {
        if (apiKey == null || apiKey.isEmpty()) {
            MineWrightMod.LOGGER.error("OpenAI API key not configured!");
            return null;
        }
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    if (responseBody == null || responseBody.isEmpty()) {
                        MineWrightMod.LOGGER.error("OpenAI API returned empty response");
                        return null;
                    }
                ...
                if (response.statusCode() == 429 || response.statusCode() >= 500) {
                    if (attempt < MAX_RETRIES - 1) {
                        ...
                        MineWrightMod.LOGGER.warn("OpenAI API request failed with status {}, retrying in {}ms (attempt {}/{})",
                ...
                MineWrightMod.LOGGER.error("Request interrupted", e);
                return null;
            } catch (Exception e) {
                if (attempt < MAX_RETRIES - 1) {
                    int delayMs = INITIAL_RETRY_DELAY_MS * (int) Math.pow(2, attempt);
                    MineWrightMod.LOGGER.warn("Error communicating with OpenAI API, retrying in {}ms (attempt {}/{})",
                    ...
                    MineWrightMod.LOGGER.error("Error communicating with OpenAI API after {} attempts", MAX_RETRIES, e);
                    return null;
                }
            }
        }
    ...
    private String parseResponse(String responseBody) {
        try {
            ...
            MineWrightMod.LOGGER.error("Unexpected OpenAI response format: {}", responseBody);
            return null;
            
        } catch (Exception e) {
            MineWrightMod.LOGGER.error("Error parsing OpenAI response", e);
            return null;
        }
    }
}
