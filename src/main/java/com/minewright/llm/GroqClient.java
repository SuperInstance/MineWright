...
public class GroqClient {
    ...
    public GroqClient() {
        this.apiKey = MineWrightConfig.OPENAI_API_KEY.get(); // Reuse same config field
    ...
    public String sendRequest(String systemPrompt, String userPrompt) {
        if (apiKey == null || apiKey.isEmpty()) {
            MineWrightMod.LOGGER.error("Groq API key is not set in the config.");
            return null;
        }
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                MineWrightMod.LOGGER.error("Groq API request failed: {} ", response.statusCode());
                MineWrightMod.LOGGER.error("Response body: {}", response.body());
                return null;
            }
        } catch (Exception e) {
            MineWrightMod.LOGGER.error("Error sending request to Groq API", e);
            return null;
        }
    }
}
